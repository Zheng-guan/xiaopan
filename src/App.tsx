...
function DriveApp({
  ...
  const [modal, setModal] = useState<
    ...
    | { type: "cancel-resume"; session: PendingUploadSession }
    | null
  >(null);
  ...
  const pendingResumesHere = pendingResumes.filter(
    (session) => session.parentId === (currentFolder?.id ?? null),
  );
  ...
  async function cancelPendingResume(session: PendingUploadSession) {
    await cancelPendingUploadSession(session, userId);
    setPendingResumes(listPendingUploadSessions(userId));
    setModal(null);
    setToast(`${session.displayName} 的未完成上传已取消，已上传分片已删除`);
          ...
          {pendingResumesHere.length > 0 && (
            <aside className="resume-upload-panel" aria-label="未完成上传">
              <div className="resume-upload-summary">
                ...
                <div>
                  <strong>此文件夹有 {pendingResumesHere.length} 个未完成上传</strong>
                  <span>重新选择原文件即可跳过已完成分片继续上传</span>
                </div>
                <button
                  className="secondary-button resume-select-button"
                  onClick={() => fileInput.current?.click()}
                >
                  <UploadCloud size={16} />
                  选择原文件继续
              ...
              <div className="resume-upload-list">
                {pendingResumesHere.map((pending) => (
                  <div className="resume-upload-session" key={pending.localStorageKey}>
                    <File size={16} />
                    <span title={pending.displayName}>{pending.displayName}</span>
                    <small>{formatBytes(pending.fileSize)}</small>
                    <button
                      className="resume-cancel-button"
                      onClick={() => setModal({ type: "cancel-resume", session: pending })}
                      title="取消上传并删除已上传分片"
                      aria-label={`取消 ${pending.displayName} 的未完成上传并删除已上传分片`}
                    >
                      <Trash2 size={15} />
                      取消
                    </button>
