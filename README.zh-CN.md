...
- 拖拽上传和多文件上传
- 基于 Cloudflare R2 的分片上传，基础分片为 16 MiB、支持网速自适应并发并可自动增大
...
- 新上传的基础分片大小为 **16 MiB**，大文件会自动增大分片，避免超过 10,000 个分片。旧版本已经创建的断点续传会话继续使用原来的 10 MiB，避免续传失效。
- 上传从 2 路并发开始，根据实际测得的上行吞吐自动调整，而不是按手机或电脑固定判断。吞吐持续提升时逐步增加到最多 6 路；发生重试时并发减半，吞吐明显下降时减少 1 路。
- 签名接口每次可为当前用户批量签发最多 16 个短时分片地址，减少 Netlify Function 往返；文件内容仍由浏览器直接传到 R2，不经过 Netlify。
- 16 MiB 是分片边界，不是带宽限速。全部使用 R2 允许的最小 5 MiB 分片会增加签名、分片上传和 ETag 操作次数，不保证提高总吞吐。
